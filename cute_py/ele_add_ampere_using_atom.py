# https://github.com/NVIDIA/cutlass/blob/main/examples/python/CuTeDSL/ampere/elementwise_add.py
import argparse
import torch
import time
...
def elementwise_add_kernel(
    ...
    print(f"[DSL INFO]   blkA = {blkA.type}")  # (16,128):(12,1)
    print(f"[DSL INFO]   blkB = {blkB.type}")
    print(f"[DSL INFO]   blkC = {blkC.type}")
    print(f"[DSL INFO]   blkCrd = {blkCrd.type}")  # cute.coord_tensor<"(0,0)", "(16,128):(1@0,1@1)">
    ...
    frgA = cute.make_fragment_like(thrA)  # ((1,(4,4)),1,1):((0,(1,12)),0,0)
    frgB = cute.make_fragment_like(thrB)
    frgC = cute.make_fragment_like(thrC)

    # blkCrd is coord_tensor ((16,128),(1,1)):((1@0,1@1),(0,0))
    # cute.coord_tensor<"(?{div=4},?{div=4})", "((1,(4,4)),1,1):((0,(1@1,1@0)),0,0)">
    thrCrd = thr_copy_C.partition_S(blkCrd)
    # frgPred = cute.make_rmem_tensor(thrCrd.shape, cutlass.Boolean)
    frgPred = cute.make_fragment(thrCrd.shape, dtype=cutlass.Boolean)

    print("[DSL INFO] Sliced Tensors per thread:")
    print(f"[DSL INFO]   tiledCopyA = {tiled_copy_A} \n")
    print(f"[DSL INFO]   thrCopyA = {thr_copy_A} \n")
    ...
    debug_tidx = 2  # 2 for M=3, N=12, or -1 for other cases
    ...
    for i in range(0, cute.size(frgPred), 1):
        ...
        if tidx == debug_tidx and bidx == 0:
            cute.printf("thrCrd[{}] = {} < {} ? {}", i, thrCrd[i], shape, val)
            # thrCrd[11] = (2,11) < (3,12) ? 1
            # thrCrd[12] = (3,8) < (3,12) ? 0
    ...
    if tidx == debug_tidx and bidx == 0:
        ...
        cute.printf("thrCrd = {} \n", thrCrd)
        # cute.print_tensor(thrCrd, verbose=True)
        cute.print_tensor(frgPred, verbose=True)
...
def elementwise_add(mA, mB, mC, copy_bits: cutlass.Constexpr = 128):
    ...
    # tiler_mn = (4x4, 32x4) = (16, 128)
    # tv_layout ((32,4),(4,4)):((64,4),(16,1))
    tiler_mn, tv_layout = cute.make_layout_tv(thr_layout, val_layout)

    print("[DSL INFO] Input Tensors:")
    print(f"[DSL INFO]   mA = {mA.type}")   # (3,12):(12,1)
    ...
    print(f"[DSL INFO]   gA = {gA.type}")  # ((16,128),(1,1)):((12,1),(0,0))
    print(f"[DSL INFO]   gB = {gB.type}")
    print(f"[DSL INFO]   gC = {gC.type}")

    idC = cute.make_identity_tensor(mC.shape)
    cC = cute.zipped_divide(idC, tiler=tiler_mn)
    print(f"[DSL INFO]   coord tensor = {cC.type}")  # ((16,128),(1,1)):((1@0,1@1),(0,0))
